...
const rawUpdateProfile = async (userId: string, profileData: any) => {
...
const validatePostData = async (args: [z.infer<typeof CreatePostSchema>]) => {
  ...
  if (!result.success) {
    throw new ValidationError('Invalid post data', result.error.issues);
  }
};
...
const rawUserByIdHandler = async (request: Request, { params }: { params: { id: string } }) => {
  const method = request.method;
...
function checkPermission(user: any, resource: string, action: string) {
