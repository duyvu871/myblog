...
export function useAuth() {
   ...
   * Sign out
   */
  const signOutUser = useCallback(async (redirectTo?: string) => {
    setIsLoading(true);
  ...
  const hasPermission = useCallback((permission: string) => {
    ...
    const userPermissions = permissions[user.role as keyof typeof permissions] || permissions[Role.USER];
   ...
   */
  const requireRole = useCallback((roles: string | string[], redirectTo?: string) => {
